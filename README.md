...
- Переключатель `Private` для файлов и папок: только создатель или доступ всем по ссылке
- Скачивание папок в `.zip` архиве

## Share-ссылки на удалённом сервере

- Если проект развёрнут не локально, `shareUrl` теперь строится по входящему домену/протоколу.
- При необходимости можно задать `PUBLIC_BASE_URL` в окружении сервера.
