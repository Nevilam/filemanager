...
def add_utf8_charset(response):
    ...
    if "charset=" in content_type:
        return response
    if content_type.startswith("text/") or content_type.startswith("application/javascript") or content_type.startswith("application/json"):
...
def get_public_item_by_code(share_code: str) -> Optional[sqlite3.Row]:
    return get_db().execute(
        """
...
def is_descendant(root_id: int, node_id: int) -> bool:
    conn = get_db()
    current_id = node_id
    visited: set[int] = set()
...
def get_share_link(item_id: int):
    ...
    if not is_item_public(row):
        return make_error("Элемент приватный, невозможно поделиться", 403)
...
def get_public_item(share_code: str):
    row = get_public_item_by_code(share_code)
    if not row:
        return make_error("Item not found", 404)

    if not is_item_public(row):
        return make_error("This item is private", 403)
...
@app.route("/api/public/<share_code>/items", methods=["GET"])
@require_auth
def list_shared_items(share_code: str):
    root = get_public_item_by_code(share_code)
    if not root:
        return make_error("Item not found", 404)
    if root["item_type"] != "folder":
        return make_error("Shared item is not a folder", 400)
    if not is_item_public(root):
        return make_error("This folder is private", 403)
    ...
    if parent_id is None:
        ...
        if not parent_row or parent_row["item_type"] != "folder":
            return make_error("Folder not found", 404)
        if not is_descendant(root_id, int(parent_id)):
            return make_error("Folder not found", 404)
        current_folder = {
            "id": str(parent_row["id"]),
            "name": parent_row["name"],
            "parentId": str(parent_row["parent_id"]) if parent_row["parent_id"] is not None else None,
        }
        target_parent_id = int(parent_row["id"])
    ...
    rows = conn.execute(
        ...
        ORDER BY CASE WHEN item_type = 'folder' THEN 0 ELSE 1 END, name COLLATE NOCASE
        """,
        (owner_id, target_parent_id),
    ).fetchall()

    public_rows = [row for row in rows if is_item_public(row)]

    return jsonify(
        {
            "ok": True,
            "items": [to_item_payload(row) for row in public_rows],
            "currentFolder": current_folder,
        }
    )


@app.route("/api/public/<share_code>/items/<int:item_id>/download", methods=["GET"])
@require_auth
def download_shared_item(share_code: str, item_id: int):
    root = get_public_item_by_code(share_code)
    if not root:
        return make_error("Item not found", 404)
    if root["item_type"] != "folder":
    ...
    if not row:
        return make_error("Item not found", 404)
    if not is_descendant(root_id, int(item_id)):
        return make_error("Item not found", 404)
    if not is_item_public(row):
        return make_error("This item is private", 403)
    ...
    archive_stream = build_folder_archive(owner_id, int(row["id"]))
    return send_file(
        archive_stream,
        as_attachment=True,
        download_name=f"{sanitize_zip_name(str(row['name']))}.zip",
